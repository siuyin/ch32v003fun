...
void big_name(uint8_t i);
void terpau() {
	printf("running terpau\n");
	uint8_t i=0;
	while(1) {
		i++;
		ssd1306_setbuf(0);
		...
		big_name(i);
		ssd1306_drawstr(0,24,"0123456789012345678901234567890",1);
		ssd1306_refresh();
		Delay_Ms(1000);
	}
}
void big_name(uint8_t i) {
	const char* ks = "Kit Siew";
	const char* sy = "Siu Yin";
	if (i%2 == 0){
		ssd1306_drawstr_sz(0,8, (char *) ks, 1, fontsize_16x16);
		return;
	}
	ssd1306_drawstr_sz(0,8, (char*) sy, 1, fontsize_16x16);
}
void gerbau(){
